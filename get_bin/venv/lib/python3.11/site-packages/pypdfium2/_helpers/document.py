...
class PdfDocument (pdfium_i.AutoCloseable):
    ...
    Attributes:
        ...
        formenv (PdfFormEnv | None):
            Form .env_example, if the document has forms and :meth:`.init_forms` was called.
    """
    ...
    def init_forms(self, config=None):
        """
        Initialize a form .env_example, if the document has forms. If already initialized, nothing will be done.
        ...
        if not raw:
            raise PdfiumError(f"Initializing form .env_example failed for document {self}.")
    ...
    def get_page(self, index):
        ...
        Note:
            This calls ``FORM_OnAfterLoadPage()`` if the document has an active form .env_example.
            The form .env_example must not be closed before the page is closed!
        """
...
class PdfFormEnv (pdfium_i.AutoCloseable):
    ...
    Attributes:
        raw (FPDF_FORMHANDLE):
            The underlying PDFium form .env_example handle.
        config (FPDF_FORMFILLINFO):
            Accompanying form configuration interface, to be kept alive.
        pdf (PdfDocument):
            Parent document this form .env_example belongs to.
    """
