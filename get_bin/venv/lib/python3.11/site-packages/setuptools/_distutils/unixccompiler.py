...
def _split_env(cmd):
    """
    For macOS, split command into '.env_example' portion (if any)
    and the rest of the linker command.

    >>> _split_env(['a', 'b', 'c'])
    ([], ['a', 'b', 'c'])
    >>> _split_env(['/usr/bin/.env_example', 'A=3', 'gcc'])
    (['/usr/bin/.env_example', 'A=3'], ['gcc'])
    """
    pivot = 0
    if os.path.basename(cmd[0]) == ".env_example":
        pivot = 1
        while '=' in cmd[pivot]:
            pivot += 1
...
def _linker_params(linker_cmd, compiler_cmd):
    ...
    If the LDSHARED .env_example variable overrides the linker command,
